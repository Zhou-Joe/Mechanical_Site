...
def batch_upload(request):
    ...
    if request.method == 'POST':
        ...
        print(f"选项: 自动上传={auto_upload}, 跳过重复={skip_duplicates}, 错误时继续={continue_on_error}")
        ...
        for i, pdf_file in enumerate(pdf_files):
            ...
            try:
                ...
                # 检查重复文件（优先检查设备和日期组合）
                if skip_duplicates:
                    # 首先检查是否已有相同设备在相同日期的报告
                    date_duplicate_found = False

                    if has_multiple_measurements and multi_data['sample_dates']:
                        for sample_date in multi_data['sample_dates']:
                            existing_date_reports = OilInspectionReport.objects.filter(
                                equipment=equipment,
                                sample_date=sample_date
                            )
                            if existing_date_reports.exists():
                                file_result['status'] = 'skipped'
                                file_result['message'] = f'跳过重复文件（设备已有{sample_date}的报告）'
                                file_result['equipment_name'] = equipment.name
                                skipped_count += 1
                                results.append(file_result)
                                print(f"跳过重复文件（日期重复）: {pdf_file.name} - 设备{equipment.name}在{sample_date}已有报告")
                                date_duplicate_found = True
                                break
                    else:
                    ...
                    # 如果找到了重复，继续下一个文件
                    if date_duplicate_found or (existing_reports.exists() if 'existing_reports' in locals() else False):
                        continue
...
def get_table_data_api(request):
    ...
    parameters = OilParameter.objects.filter(
        report__equipment_id=equipment_id,
        report__report_date__gte=start_date
    ).order_by('-report__report_date', 'parameter_name')
    ...
    for param in parameters:
        table_data.append({
            ...
            'sample_date': param.report.sample_date.strftime('%Y-%m-%d'),
            'report_number': param.report.report_number or '',
            'parameter_name': param.parameter_name,
            'parameter_value': param.parameter_value,
            'unit': param.unit,
            'standard_range': param.standard_range or '',
            'is_normal': param.is_normal,
            'category': classify_parameter_category(param.parameter_name)
        })
